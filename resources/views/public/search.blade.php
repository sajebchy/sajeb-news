...
<main class="max-w-[1200px] mx-auto px-4 md:px-6 py-8">
    ...
    {{-- ===== Search Header ===== --}}
    <div class="mb-8">
        <h1 class="text-3xl md:text-4xl font-bold text-on-surface mb-1" style="font-family:'Noto Serif Bengali',serif;">
            @if($query)
                ফলাফল: "{{ $query }}"
            @else
            ...
            <div class="bg-surface-container-low p-5 rounded-xl">
                ...
                <div class="space-y-3">
                    ...
                    <a href="{{ route('news.show', $t->slug) }}" class="flex gap-3 items-start group {{ $i > 0 ? 'border-t border-outline-variant pt-3' : '' }}">
                        ...
                        <p class="text-sm text-on-surface-variant group-hover:text-primary transition-colors line-clamp-2" style="font-family:'Noto Serif Bengali',serif;">
                            {{ $t->title }}
                        </p>
                    </a>
                    @endforeach
        ...
        <section class="flex-1 min-w-0">
            ...
            @if($query)
                ...
                <article class="flex flex-col sm:flex-row gap-5 p-4 mb-4 border border-outline-variant rounded-xl bg-surface hover:shadow-lg transition-shadow duration-300 group">
                    ...
                    <div class="w-full sm:w-1/3 flex-shrink-0 overflow-hidden rounded-lg" style="min-height:160px;max-height:180px;">
                        <img class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                             src="{{ $article->featured_image ? Storage::url($article->featured_image) : 'https://picsum.photos/seed/'.$article->id.'/600/400' }}"
                             alt="{{ $article->title }}">
                    </div>
                ...
                <div class="text-center py-24">
                    ...
                    <h3 class="text-xl font-semibold text-on-surface-variant mt-4" style="font-family:'Noto Serif Bengali',serif;">
